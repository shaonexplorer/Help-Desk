...
export function TimeRangeSelector({ value, onChange, className }: TimeRangeSelectorProps) {
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      ...
      {open && (
        <div
          className="absolute right-0 z-50 mt-1 min-w-[160px] rounded-lg border border-border bg-popover p-1 shadow-lg"
          role="listbox"
        ...
        >
          {RANGES.map(range => (
            ...
            >
              ...
              {range.value === value && <Check className="size-4 text-primary" aria-hidden="true" />}
            </button>
          ))}
        </div>
      )}
